"""Find pragmatism quotes not yet translated."""
import re, json
...
for i, cat in enumerate(cats):
    if cat == 'pragmatism' and i < len(ens) and i < len(pts):
        en = unescape_dart(ens[i])
        pt = unescape_dart(pts[i])
        if en == pt:
            missing.append(en)

with open('c:/Users/lalli/Flutter/coach_phrase_app/missing_prag.json', 'w', encoding='utf-8') as f:
    json.dump(missing, f, ensure_ascii=False, indent=2)

print(f'Missing pragmatism: {len(missing)}')
